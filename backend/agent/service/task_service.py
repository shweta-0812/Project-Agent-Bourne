...
def create_plan_task(agent):
    create_plan_task = Task(
        ...
        expected_output=(
            ...
            "2. expected_output: The expected output when the task is complete. The output can be a file or text needed by other tasks or indicating the success or failure status of the current task"  # noqa: E501
        ),
        agent=agent,
    )
...
def create_review_task(agent, output_file):
    review_plan = Task(
        description=(
            ...
            "Also, confirm that there are no missing elements in the plan that could prevent it from achieving the goal. "  # noqa: E501
...
def create_team_task(
    ...
    create_team = Task(
        ...
        expected_output=(
            ...
            "4. tools: selected list of tools from the given tools required by this role."
        ),
        output_json=Profile,
        output_file=output_file,
        agent=agent,
